...
def allBehavoirs():
	...
	f= open("/home/myw/DroidBox_4.1.1/feature-format.txt","r")
	s = f.read()
	li = s.strip().split(',')
...
def testrVector(dirr):
	for s in os.listdir(dirr):
		...
		fi = open("/home/myw/DroidBox_4.1.1/test_feature.txt",'a')
		value = str(vector)
		value = value.replace('[','')
		value = value.replace(']','')
		fi.write(value+'\n')
...
if __name__ == "__main__":
	allBehavoirs()
	testrVector("/media/myw/Windows8_OS/testAPK/malwareAPK/test-apk-malware-json")
